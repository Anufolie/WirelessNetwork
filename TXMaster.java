...
public class TXMaster extends JE802_11MacAlgorithm {
	...
	private PID pidAck = new PID (target,1.2, 0.9, 0.9);
	
	private long prevTx, prevAck;

	public TXMaster(String name, JE802_11Mac mac) {
		super(name, mac);
	...
	public void compute() {
		...
		if (!Double.isNaN(ackTxRatio)) {
			...
			if (output > 1 || output < 0) {
				CWmin = Math.min(16, CWmin * 2); // increase window exponentially
				CWmax = Math.min(1024, CWmax * 2);
			} 
			else if (output <= 1) {
